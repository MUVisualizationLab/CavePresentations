...
public class slideMaster : MonoBehaviour {
	...
	void Update () {
		...
		if (slidesOn)
		{
			if (MVR.DeviceMgr.IsWandButtonToggled(1) || MVR.DeviceMgr.IsKeyToggled(MVR.VRK_C))
				deactivate();
		}
		else
		{
			if (MVR.DeviceMgr.IsWandButtonToggled(2) || MVR.DeviceMgr.IsKeyToggled(MVR.VRK_C))
				reactivate();
			return;
		}
	...
	public void requestSlide(int newMainSlide){
        ...
        if (!Cluster.IsClient) {
            notesText.text = transform.GetChild(newMainSlide).GetComponent<slideMeta>().note;
        }
	...
	IEnumerator setStatusAnim(int slideNum, int newPosition){
		...
		if (newPosition == 2) {
			currentSlide = slideNum;
			slide.GetComponent<drawingManager>().enableDrawing();
		}
